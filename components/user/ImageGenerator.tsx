...
const ImageGenerator: React.FC<ImageGeneratorProps> = ({ user, onUserUpdate }) => {
    ...
    const handleGenerate = async () => {
        ...
        if (user.image_requests >= 5) {
             setError("شما به محدودیت روزانه تولید عکس رسیده‌اید.");
             onUserUpdate();
             return;
        }
    ...
    const isLimitReached = user.image_requests >= 5;

    return (
        <div className="max-w-3xl mx-auto animate-fade-in">
            <h1 className="text-3xl font-bold text-white mb-2">{uploadedImage ? 'ویرایشگر عکس' : 'تولیدکننده عکس'}</h1>
            <p className="text-sm text-slate-400 mb-6">محدودیت روزانه: {user.image_requests} / 5. {uploadedImage ? 'دستور ویرایش رو بنویس.' : 'عکسی که می‌خوای بسازی رو توصیف کن.'}</p>
             ...
             {isLimitReached && !isLoading && !currentImageUrl && (
                <div className="bg-yellow-900/50 border border-yellow-700 text-yellow-300 px-4 py-3 rounded-lg text-center">
                    شما به محدودیت روزانه تولید عکس رسیده‌اید. فردا دوباره سر بزنید!
                </div>
            )}
