...
const FreeChat: React.FC<FreeChatProps> = ({ user, onUserUpdate }) => {
    ...
    const handleSend = async () => {
        ...
        if (user.chat_messages >= 10) {
            alert("شما به محدودیت پیام روزانه خود رسیده‌اید.");
            onUserUpdate();
            return;
        }
            ...
            <div className="space-y-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        >
                            {msg.text ? 
                             <div className="prose prose-invert prose-p:my-0" dangerouslySetInnerHTML={{ __html: msg.text }} />
