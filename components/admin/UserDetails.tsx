...
const UserDetails: React.FC<UserDetailsProps> = ({ user, onBack, onUpdate }) => {
    ...
    const handleDeleteScenario = async (id: number) => {
        if (window.confirm('این سناریو حذف شود؟ با این کار یک کپشن برای کاربر تولید می‌شود.')) {
    ...
    return (
        <div className="animate-fade-in">
            <button onClick={onBack} className="flex items-center text-violet-400 hover:text-violet-300 mb-4">
